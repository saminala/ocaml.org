          ...
          let (path : Ocamlorg_frontend.Breadcrumbs.path_item list) =
            if doc.module_path != [] then
              ...
              let first_path_item_title = match first_path_item with
                ...
                | `ClassType s -> s
              in
              (* TODO: we need to know if it's a standalone page, if so, we can't find the lib path item, but need to find
                 the docs path item *)
              ...
              match library_path_item with
                ...
                | None -> Page "" :: path (* TODO: as soon as we have an identifier for
                        standalone docs pages, we should use that here instead of "" *)
            else
              []
          in
          let package_meta = package_meta t package in
